...
const MegaSearch: React.FC<MegaSearchProps> = ({
  ...
  return <div ref={megaSearchRef} className="search-container p-6 relative z-10">
      ...
      <div className="relative mb-6">
        <div className="flex rounded-lg overflow-hidden border border-gray-300">
          <div className="flex-1 relative">
            ...
            <Input ref={megaInputRef} type="text" placeholder="Type anything - programs, schools, scholarships..." value={searchQuery} onChange={handleInputChange} onKeyPress={onKeyPress} onFocus={handleInputFocus} className="pl-10 pr-4 py-3 text-lg border-0 rounded-none focus:ring-0 focus:border-transparent" />
          </div>
        ...
        {showMegaDropdown && <div className="fixed left-1/2 transform -translate-x-1/2 w-full max-w-4xl mt-1 bg-white border border-gray-300 rounded-lg shadow-xl z-[9999] max-h-96 overflow-y-auto">
            <div className="p-4">
              <div className="mb-6">
                <h4 className="text-sm font-semibold text-gray-500 uppercase mb-3">Search in</h4>
                <div className="flex flex-wrap gap-2">
                  {megaSearchCategories.map(type => <div key={type.name} className={`inline-flex items-center gap-2 px-3 py-2 rounded-full border cursor-pointer transition-all hover:shadow-md ${selectedCategory === type.name ? 'border-primary bg-primary/10 text-primary' : 'border-gray-200 bg-white hover:border-gray-300'}`} onClick={() => onCategorySelect(type)}>
              ...
              <div className="mb-6">
                ...
                <div className="space-y-1">
                  {recentSearches.map(search => <div key={search} className="flex items-center gap-2 px-2 py-1 hover:bg-gray-100 cursor-pointer rounded text-sm" onClick={() => onPopularSearchClick(search)}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {popularSearches.map(search => <Badge key={search} variant="secondary" className="cursor-pointer hover:bg-primary hover:text-primary-foreground text-xs px-2 py-1" onClick={() => onPopularSearchClick(search)}>
                      {search}
                    </Badge>)}
                </div>
              </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {megaSearchCategories.map(type => <div key={type.name} className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border cursor-pointer transition-all hover:shadow-md text-xs ${selectedCategory === type.name ? 'border-primary bg-primary/10 text-primary' : 'border-gray-200 bg-white hover:border-gray-300'}`} onClick={() => onCategorySelect(type)}>
